...
function serverFactory(core, messageHandler, Logger) {
    ...
    Server.prototype.send = function(data) {
        ...
        var current_outport = this.outPort;
        ...
        if (data.isefi == true) {
            current_outport = this.outportEFI;
        }
        logger.info('current OutPort => %d'.format(current_outport));
        this.server.send(packetBuffer, 0, packetBuffer.length, current_outport,
