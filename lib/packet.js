...
function PacketFactory(protocol, configuration, _) {
    ...
    var packetFunctions = {
        createProxyDhcpAck: function(clientPacket, bootFileName) {
            ...
            packet.siaddr = configuration.get('server');

            // Not necessary, at least on vbox, but perhaps other clients will require these fields?
            packet.sname = configuration.get('server');
            packet.options.broadcastAddress = configuration.get('broadcastaddr');
            packet.options.serverIdentifier = configuration.get('server');
            ...
            var packetBuffer = createPacketBuffer(packet);
            return {
